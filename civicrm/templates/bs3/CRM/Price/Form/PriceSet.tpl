...
*}
<div class="col-xs-12">
	<fieldset id="priceset" class="crm-section price_set-section form-group clearfix">
		...
		{foreach from=$priceSet.fields item=element key=field_id}
			...
			{if $element.visibility EQ 'public' || ($element.visibility EQ 'admin' && $adminFld EQ true) || $context eq 'standalone' || $context eq 'advanced' || $context eq 'search' || $context eq 'participant' || $context eq 'dashboard' || $action eq 1024}
				...
				{else}
					...
					<div class="label col-xs-12 col-sm-6">{$form.$element_name.label}</div>
					<div class="content {$element.name}-content col-xs-12 col-sm-6">{$form.$element_name.html|crmAddClass:'form-control'}
