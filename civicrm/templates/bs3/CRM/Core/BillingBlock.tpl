...
<div id="payment_information" class="row">
  <div class="col-xs-12 col-md-6">
	  {if $paymentFields|@count && (!$form.$expressButtonName || $paymentProcessor.payment_processor_type EQ 'PayPal')}
			<fieldset class="panel billing_mode-group {$paymentTypeName}_info-group">
				<legend>
				{$paymentTypeLabel}
				</legend>
				...
				{foreach from=$paymentFields item=paymentField}
					...
					<div class="crm-section {$form.$paymentField.name}-section clearfix">
						...
						<div class="content">{$form.$paymentField.html}
							...
							{if $paymentField == 'credit_card_type'}
								<br>
								<div class="crm-credit_card_type-icons" style="margin: 0 auto;"></div>
								<br>
							{/if}
						</div>
					</div>
	  ...
	  {if $billingDetailsFields|@count && $paymentProcessor.payment_processor_type neq 'PayPal_Express'}
			...
			<fieldset class="panel billing_name_address-group">
				...
				{foreach from=$billingDetailsFields item=billingField}
					...
					<div class="crm-section {$form.$billingField.name}-section clearfix">
					<div class="label">{$form.$billingField.label}
						{if $requiredPaymentFields.$name}<span class="crm-marker" title="{ts}This field is required.{/ts}">*</span>{/if}
					</div>
					<div class="content">{$form.$billingField.html|crmAddClass:'form-control'}</div>
					</div>
				{/foreach}
				</div>
			</fieldset>
